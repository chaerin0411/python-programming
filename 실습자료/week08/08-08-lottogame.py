'08-08-lottogame.py'
'Mini Project 8 : 로또 복권 당첨 모의 실험'
'우리나라의 로또 당첨 순위를 알아보고 모든 등수와 등수외의 복권 번호를 의도적으로 만들어 출력해 보는 프로그램을 작성해 보자'
'딕셔너리와 튜플, 리스트, 조건, while과 for 반복, 모듈 random의 choice(), randint() 함수 활용'
''' 딕셔너리의 키(key)로 번호 6개 중 맞춘 개수로 정한다. 즉 키가 6은 1등으로 값(value)은
    ('1등'. 1000000000)처럼 등수와 상금의 튜플로 만든다. 다만, 5개를 맞히면 다시 키 1과 0으로,
    2등과 3등으로 나눈다.실제 로또에서 3등 이상의 상금은 매회 달라지므로 다음처럼 정액 값으로 저장한다.
    # 로또 복권 순위 판정에 사용할 딕셔너리
    rank = { 6: ('1등', 1000000000), # 1등 10억 원, 맞춘 번호 6개
