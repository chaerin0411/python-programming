'08-07-rockgame.py'
'Mini Project 7 : 가위바위보 게임'
'조건문 if, 리스트와 딕셔너리, 모듈 random의 choice() 함수, while문 활용'
''' 다음 딕셔너리 madewin은 가위바위보에서 승패가 나눠지는 세 가지 경우를 항목으로 구성했다.
    구성된 항목 앞의 키에는 승자, 값에는 패자의 손 모양을 기술한다.
...
while True:
    ...
    if not (0 <= num <= 3): # 범위의 값이 아니면 다시 시작
