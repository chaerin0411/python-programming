'13-08-texteditorwithtk.py'

# os.path.basename 사용
import os
# 기본적인 웨젯 Frame, Text, Menu 등 사용
from tkinter import *
...
class TextEditor():
	...
	def __init__(self, root):
		...
		root.protocol("VM_DELETE_WINDOW", self.file_quit)
		# 메뉴 생성
		self.make_menu()
		# 단축키 이벤트를 메소드와 연결
		self.bind_events()
	...
	def save_if_modified(self):
		...
		if self.editor.edit_modified():
			...
			if response: # 저장하도록 저장 메소드 호출
				...
				return response # None = cancle/about, False = no/discard
		else: # 수정되지 않았으면
			return True
	...
	def file_quit(self, event=None):
		...
		result = self.save_if_modified()
		if result != None: # 기준 편집하던 것이 해결됐으면
